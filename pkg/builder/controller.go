...
type informersInternal interface {
	GetInformerOrCreateWithTransformerIfNecessary(ctx context.Context, obj client.Object, transformer cache.ObjectTransformerFunc) (cache.Informer, error)
}
...
func (blder *Builder) installMetadataFilterForKindSource(kind *source.Kind, filter *metadataFilter) {
	...
	internalMap, ok := informerMap.(informersInternal)
	if !ok {
		return
	}
	...
	// Creates a SharedIndexInformer whose listwatch is wrapped with a
	// transformer applied to all objects to strip out managed fields and
	// annotations
	_, _ = internalMap.GetInformerOrCreateWithTransformerIfNecessary(context.Background(), kind.Type, func(o runtime.Object) runtime.Object {
		stripFields(o)
		return o
	})
}
