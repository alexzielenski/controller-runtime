...
func (m *InformersMap) Get(ctx context.Context, gvk schema.GroupVersionKind, obj runtime.Object, transformer ObjectTransformerFunc) (bool, *MapEntry, error) {
	switch obj.(type) {
	case *unstructured.Unstructured:
		return m.unstructured.Get(ctx, gvk, obj, transformer)
	case *unstructured.UnstructuredList:
		return m.unstructured.Get(ctx, gvk, obj, transformer)
	case *metav1.PartialObjectMetadata:
		return m.metadata.Get(ctx, gvk, obj, transformer)
	case *metav1.PartialObjectMetadataList:
		return m.metadata.Get(ctx, gvk, obj, transformer)
	default:
		return m.structured.Get(ctx, gvk, obj, transformer)
	}
}
